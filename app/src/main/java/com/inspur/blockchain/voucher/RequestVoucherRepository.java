 ...
 * @author lichun
 */
public class RequestVoucherRepository extends BaseRepository {
    ...
    public LiveData<JSONObject> requestVoucher(ArrayMap<String,String> params){
        ...
        defaultHeaders().url(UrlConfig.REQUEST_VOUCHER_URL).body(params).post().build().requestJson(new NetCallback<JSONObject>() {
            @Override
